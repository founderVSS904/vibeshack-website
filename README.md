...
- `CRON_SECRET` for the protected booking reminder and hold-watchdog cron routes

Stripe Checkout receives the customer's email as `receipt_email`, and the webhook sends both the customer confirmation/prep emails and the internal founder booking notification after payment. Subscribe the Stripe webhook to both `checkout.session.completed` and `checkout.session.expired`; expired-session delivery removes abandoned checkout holds promptly. Holds also carry their own expiration time, so an abandoned hold never blocks availability if webhook delivery is delayed. After payment, the webhook renews the hold through the booking date until the confirmed Calendar event is safely written. The Vercel reminder cron checks website-created Google Calendar events every hour and sends the customer 24-hour reminder once per booking.

The protected booking-hold watchdog runs every 10 minutes. It scans a bounded window of website-created Stripe sessions, rechecks each candidate directly with Stripe, and reconciles only expired, unpaid holds. Paid or completed sessions are always protected. Cleanup attempts every Calendar artifact, records session-specific failures in Stripe metadata, and emails the founder when a hold cannot be repaired safely. Delivered session alerts use a six-hour Stripe-backed cooldown. A pre-send reservation prevents rapid repeats and becomes retryable after 30 minutes if delivery fails. Scan-wide failures return an error on every run for Vercel monitoring and use a fixed six-hour email window, so a provider outage cannot send an email every 10 minutes.
