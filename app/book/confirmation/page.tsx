          ...
          <div className="my-8 divide-y divide-white/10 rounded-lg border border-white/15 px-5">
            {result.summary.sessions.map((session, index) => (
              <div key={index} className="py-5">
                ...
                {session.addOns.map((addOn) => <p key={addOn.name} className="mt-3 text-sm text-zinc-300">{addOn.name}: {addOn.amount === 0 ? 'No charge' : addOn.billing === 'session' ? `$${addOn.amount.toFixed(2)} per session` : `$${addOn.hourlyRate}/hr · $${addOn.amount.toFixed(2)}`}</p>)}
              </div>
            ))}
